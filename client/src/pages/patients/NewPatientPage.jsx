...
const INITIAL = {
  ...
  medicalHistory: { allergies: '', conditions: '', medications: '', notes: '' },
};
...
export default function NewPatientPage() {
  ...
  // Strip dashes/spaces from IC so user can type 990101-14-1234 or 990101141234
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (err) => {
      const msg = err.response?.data?.details?.[0] || err.response?.data?.message || 'Failed to create patient';
      toast.error(msg);
    },
  });
  ...
  // Display IC with dashes: 990101-14-1234
  const displayIC = (raw) => {
        ...
        <Card className="p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-gray-400">
              {form.idType === 'ic'
                ? `${form.idNumber.length}/12 digits — dashes are added automatically`
